...
mod tests {
    ...
    fn the_full_album_response_names_the_whole_tracklist() {
        ...
        assert_eq!(facts.tracks.first().map(String::as_str), Some("One More Time"));
    ...
    fn the_track_count_is_never_less_than_the_tracks_listed() {
        ...
        assert_eq!(facts.nb_tracks, 3, "the listed tracks outnumbered the stated count");
    }
    ...
    fn the_parsers_still_match_the_live_services() {
        ...
        let facts = found.facts.expect("no album facts: tracks.data went missing");
        assert_eq!(facts.nb_tracks, 14, "Discovery is a fourteen-track record");
        assert_eq!(facts.tracks.len(), 14, "the tracklist did not arrive in full");
        assert_eq!(facts.record_type, "album");
    }
