...
**Before 2026-08-24 there was no keystore, so builds went out `--debug`.** Two
consequences, kept because they are what the signing work below bought:

* The package was `com.dylangrowcoot.vapormusic.debug`, which installs *beside*
  a release build rather than over it.
* The APK carried full native debug symbols: **591 MB** measured 2026-08-20,
  against 41 MB for the signed release APK in `v2.0.0-rc.10`. The debug one
  installs to a Pixel 9 over wireless ADB in 58 s, which is survivable for
  testing and absurd for shipping.

**Wired 2026-08-24, and all three are done.** The keystore was Dylan's to make,
because making one sets a password; the four secrets below went on the
repository the same day. `v2.0.0-rc.7` published the first signed release APK
on 2026-08-25 and every tag since has published another, which is the
difference between this being configured and this working.

1. A keystore, generated by Dylan with the command below. Done 2026-08-24.
...
Separate from both of the above. The pair described here was generated
2026-08-24; the rotation history underneath it is worth reading before touching
any of this:
...
The first fix read the public half off `~/.tauri/vapor-music-updater.key.pub`
and pasted it in — `E17781A5EB1BC8D6`. It lasted hours. The pair from the
rotation carried a password nobody could reproduce, and `tauri build` cannot
sign with a key it cannot open, so a **fresh pair was generated the same day
with an empty password**. That is the pair `plugins.updater.pubkey` carries:
`5BA4A4817CBE6728`. `release.yml`'s `verify` job fails the run if the
2026-08-22 string ever comes back.

Written out at this length because the short version — "the config was stale,
one line fixed it" — has been recorded twice and was wrong both times.
...
The `TAURI_SIGNING_PRIVATE_KEY` repository secret holds the **contents of the
key file**, copied from the file rather than echoed. It went in on 2026-08-24,
and every tag since has produced a `.sig` beside each bundle — which is what
...
A prerelease does not satisfy this. `releases/latest` skips them, so each `-rc`
tag published a `latest.json` that nothing would ever fetch; the first tag
without the suffix is what makes the URL answer. While it does not, it returns
404, the check fails, and the app logs one line and carries on — intended,
not a fault.
...
- [x] ~~**Android keystore created, and the four repository secrets set.**~~
