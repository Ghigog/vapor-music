...
@onready var placeholder_icon: TextureRect = $Margin/VBox/ArtRatio/PlaceholderPanel/IconLabel
...
## Guards against a stale async decode landing after the card has been
## reused for a different track (cards are reused across setup() calls in
## the vibe list).
var _art_request_path: String = ""
...
func setup(href: String, meta: Dictionary, cost: float, type: String, is_selected: bool, is_ai_preferred: bool, current_bpm: float, predicted_transition: String) -> void:
	...
	_art_request_path = img_path
	_apply_art_state(false)
	if not img_path.is_empty():
		ThumbnailService.request(img_path, func(tex: Texture2D) -> void:
			if not is_instance_valid(self) or _art_request_path != img_path:
				return
			if tex:
				art_rect.texture = tex
				_apply_art_state(true)
		, 256)
...
## Toggles between the placeholder (no art yet/failed decode) and the
## bordered-art look. Called synchronously (placeholder) before an async
## decode, then again if it resolves to a real texture.
func _apply_art_state(has_img: bool) -> void:
	var theme = ThemeManager.current_theme
	if not has_img:
		...
		placeholder_icon.texture = ICON_SONG
		placeholder_icon.self_modulate = theme.TEXT_TERTIARY
	else:
