...
**What works instead:** `-dontshrink`, `-dontoptimize`, `-dontobfuscate`,
`-dontwarn **` and `-ignorewarnings` in `app/proguard-rules.pro`. R8 still runs
as a dexer and changes nothing on the way through, which is the outcome that
was wanted. `-dontwarn **` prevents R8 from failing on missing desktop Java
classes (such as `java.beans.*` and `javax.lang.model.*`) referenced by
transitive dependencies. That file is not regenerated by the CLI and the
template already passes it to `proguardFiles`.
