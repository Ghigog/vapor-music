...
| ~~MIG-023~~ | ~~The new shell has no media controls on any platform.~~ **Done** — `souvlaki` covers macOS, Windows and Linux from `media.rs`, which is the single cross-platform answer MIG-022 said this should be rather than three platform ports. Play, pause, toggle, next and previous are answered; **stop is deliberately not**, since the shell's stop forgets what was playing and answering it with a pause would leave the transport claiming a finished track. Seeking from Control Center is unanswered — it needs a position the mixer agrees with, which is more than a mapping. Registered on the main thread, only when there is an audio device, and published from the supervisor with unchanged states dropped so a moving position is not four D-Bus round trips a second. **Compiled on all three, pressed on none — TD-54.** | 4 | MIG-022 |
