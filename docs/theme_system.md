...
When using custom shaders or custom border configurations that rely on specific StyleBox parameters (such as the blended borders in [base_stylebox.tres](addons/shader/base_stylebox.tres) required by [glass_panel.gdshader](addons/shader/glass_panel.gdshader)), components should preserve the editor-configured StyleBox instead of replacing it with a new one generated by `ThemeManager`.
