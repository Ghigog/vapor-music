...
| TD-09 | **A track is still decoded whole into memory, now at half the size.** Samples are stored as `i16`, so a five-minute track at 48 kHz costs 55 MB rather than 110, and two decks near a transition 110 MB rather than 220. The sources are lossy AAC and MP3 whose noise floor sits far above 16 bit. **Streaming is still the real fix** and would take it to about 1 MB a deck: it needs a decoder thread, a windowed source the WSOLA search can index into, and new seek semantics. Deliberately not attempted at the end of a long session — done badly it reintroduces dropouts, which is the one thing the audio path has been careful about. | `deck.rs` |
