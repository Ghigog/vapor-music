...
func _apply_styles() -> void:
	...
	action_btn.add_theme_stylebox_override("pressed", ThemeManager.make_cta_button(true))
	action_btn.add_theme_color_override("font_color", theme.ACCENT_CORE)
```

---

## 5. Preserving Editor StyleBoxes (Custom Shaders)

When using custom shaders or custom border configurations that rely on specific StyleBox parameters (such as the blended borders in [base_stylebox.tres](file:///Users/dylangrowcoot/Documents/Personal%20Apps/vapor-music/addons/shader/base_stylebox.tres) required by [glass_panel.gdshader](file:///Users/dylangrowcoot/Documents/Personal%20Apps/vapor-music/addons/shader/glass_panel.gdshader)), components should preserve the editor-configured StyleBox instead of replacing it with a new one generated by `ThemeManager`.

**Integration Pattern:**
1. In `_ready()`, check for and cache a duplicate of the editor-configured stylebox override, and apply it immediately:
   ```gdscript
   ...
   func _ready() -> void:
       ...
       _apply_styles()
   ```
2. In `_apply_styles()`, if the stylebox was cached, directly update its color properties based on the active theme:
   ```gdscript
   func _apply_styles() -> void:
       if _custom_stylebox:
           _custom_stylebox.bg_color = Color(theme.BG_GLASS.r, theme.BG_GLASS.g, theme.BG_GLASS.b, _custom_stylebox.bg_color.a)
           _custom_stylebox.border_color = Color(theme.GLASS_BORDER.r, theme.GLASS_BORDER.g, theme.GLASS_BORDER.b, _custom_stylebox.border_color.a)
       else:
           add_theme_stylebox_override("panel", ThemeManager.make_glass_panel(theme.RADIUS_MD))
   ```
