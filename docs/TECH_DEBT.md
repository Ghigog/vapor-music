...
| ~~TD-31~~ | ~~No drag and drop.~~ **Partly done, and the rest is blocked** — the Queue reorders by dragging, using the platform's own HTML5 drag events rather than `dnd-kit`; explicit up/down buttons cover the keyboard, which a pointer-only implementation would have needed anyway. Dragging tracks *onto* playlists cannot be built because **there is nothing to drop onto** — see TD-44. | TD-44 |
...
| TD-44 | **Playlists have no view.** The design's Library has four tabs — Albums, Songs, Artists, **Playlists** — and the app has two. A playlist can be added to, through the `Add to…` dropdown in the Songs selection bar, and that is the whole of it: no screen lists playlists, opens one, plays one, renames or deletes one, and `create_playlist` is bound in `core.ts` but **called from nowhere**, so one cannot even be made. `playlist.rs` is fully ported and tested (445 lines), and the shell exposes three commands for it; this is a missing UI, not missing logic. Found while looking at TD-31, which it blocks. Note this is invisible to `tests/command_bindings.rs` (TD-37): that checks a command is *bindable* from `core.ts`, not that any screen calls it. | TD-30, TD-31 |
